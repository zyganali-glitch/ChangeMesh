"""ChangeMesh Live GCP Access Smoke Test Script.

This is an explicitly authorized standalone smoke script executed via:
`uv run python scripts/cmd.py integration --live-write-danger` or
`python tests/test_gcp_access.py`.

It performs live cloud writes to Firestore, Pub/Sub, and Cloud Run, and must NEVER
be run implicitly or collected by pytest during standard non-live test execution.
"""

__test__ = False  # Instruct pytest collector to ignore this standalone script
...
def check_firestore_access(project: str) -> None:
...
def check_pubsub_access(project: str) -> None:
...
def check_cloud_run_access(project: str) -> None:
    ...
    service.template.containers = [
        run_v2.Container(image="us-docker.pkg.dev/cloudrun/container/hello")
    ]

    request = run_v2.CreateServiceRequest(parent=parent, service_id=service_id, service=service)
...
def run_tests() -> None:
    ...
    try:
        ...
        check_firestore_access(project)
        check_pubsub_access(project)
        check_cloud_run_access(project)
