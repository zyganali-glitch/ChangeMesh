...
> **Status:** `P-00..P-19 DONE; P-20 IN_PROGRESS (P-20.01 IMPLEMENTED AND VERIFIED)`
> **Produced by:** P-04 through P-20.01
> **Date:** 2026-08-18
> **Implementation state:** Architecture design is complete (P-04). Domain contracts and machine conventions (P-05) are implemented and frozen. Runtime dependency freeze (P-06) is complete. Change Orchestrator ADK skeleton and specialized agent fleet (P-07) are implemented in `src/agents/`. Bounded Gemini Model Client (P-08) is implemented in `src/core/`. Pub/Sub event backbone (P-09) is implemented in `events/` and `src/evidence/`. Multi-tenant state repository, idempotency manager, saga checkpoints, and teardown privacy (P-10) are implemented in `src/orchestrator/`. Memory trust layer (P-11), agent capability passport registry (P-12), ShadowLab runner (P-13), Policy Guardian reversibility gate (P-14), Impact Scout blast radius analysis (P-15), deterministic policy engine (P-16), migration plan/manifest generators (P-17), evidence audit & deterministic reconciliation (P-18), and Release Steward 5-point idempotent writebacks (P-19) are implemented and verified. End-to-end ChangeLifecycle saga orchestrator (P-20.01) is implemented in `src/orchestrator/orchestrator_saga.py` (`ChangeSagaOrchestrator`, `SagaExecutionResult`), coordinating all 8 lifecycle stages with event-driven causation, persistent state records, optimistic concurrency, and zero human authority bypass. Remaining phases (P-20.02+, P-21..P-32) remain `PLANNED` or `PENDING`.
...
Domain contracts (`domain/contracts/`) are `IMPLEMENTED` and verified (P-05). Change Orchestrator ADK intake skeleton (`src/agents/change_orchestrator.py`) is `IMPLEMENTED` under P-07.01. Six specialized ADK agent fleet definitions and bounded tool/instruction contracts (`src/agents/`) are `IMPLEMENTED` under P-07.02. Deterministic routing and delegation engine (`src/agents/router.py`) is `IMPLEMENTED` under P-07.03. Multi-agent branch coordination, parallel execution, single-writer aggregation, and sequential fallback (`src/agents/coordinator.py`) is `IMPLEMENTED` under P-07.04. All remaining persistence, provider adapter, API entrypoint, specialized business logic (P-15–P-19), and UI target packages listed below remain `PLANNED` for implementation in their respective owning phases.
...
|---|---|---|---|---|---|---|
| **Domain Contracts** | `domain/contracts/` | Versioned schemas, enums, ports/interfaces, domain types (IMPLEMENTED in P-05) | Core (depended upon by all) | Standard library primitives, other domain contracts | Google SDK, ADK, Firestore, PubSub, GitHub, UI, fixtures | Provider-neutral |
...
| **Change Orchestrator** | `src/agents/change_orchestrator.py`, `src/orchestrator/orchestrator_saga.py` | ADK intake skeleton implemented (P-07.01); definition metadata implemented (P-07.02); deterministic routing/delegation implemented (P-07.03 via `src/agents/router.py`); multi-agent branch coordination, parallel execution, single-writer aggregation, and sequential fallback implemented (P-07.04 via `src/agents/coordinator.py`); end-to-end 8-stage lifecycle saga orchestrator implemented in `src/orchestrator/orchestrator_saga.py` (`ChangeSagaOrchestrator`, P-20.01 IMPLEMENTED); pause/resume/recovery PLANNED under P-20.02+ | Inward (depends on domain contracts) | Domain contracts, Deterministic Router, Branch Coordinator, SagaStateRepository | Direct Firestore/PubSub client calls bypassing ports | Provider-specific (ADK) |
| **Saga State Repository** | `src/orchestrator/state_repository.py`, `src/orchestrator/in_memory_repository.py` | Durable multi-tenant operational state persistence, optimistic concurrency, idempotency reservation, and saga checkpointing (IMPLEMENTED in P-10.02, P-20.01) | Inward (depends on domain contracts) | Domain contracts, Firestore adapter port | Direct Firestore SDK in core, UI, fixtures, GitHub SDK | Provider-neutral core + Firestore persistence adapter |
| **Impact Scout** | `src/agents/impact_scout.py` (ADK agent), `src/git/impact_scout.py` (Git analysis) | Read-only blast-radius, repository overlap, parallel-change conflict detection (ADK definition IMPLEMENTED in P-07.02; Git scanner, graph traverser, and blast radius merger IMPLEMENTED in P-15) | Inward (depends on domain contracts) | Domain contracts, repository adapter port | Direct GitHub/GitLab SDK in core, UI | Provider-neutral core + repository adapter |
| **Policy Guardian** | `src/agents/policy_guardian.py`, `src/policy/policy_engine.py`, `src/gate/policy_guardian_gate.py` | Deterministic and model-assisted policy checks, safety pre-checks, prompt/input privacy minimization, and approval compression (ADK definition IMPLEMENTED in P-07.02; ZK-PRIV-001 privacy boundary IMPLEMENTED in P-08.03; reversibility classifier and gate IMPLEMENTED in P-14; deterministic policy engine and injection detector IMPLEMENTED in P-16) | Inward (depends on domain contracts) | Domain contracts, ShadowLab Auth | Direct model SDK calls, UI | Provider-neutral core with provider-specific model adapter |
| **Migration Engineer** | `src/agents/migration_engineer.py`, `src/migration/` | Scoped artifact generation, expand-migrate-contract planning, worktree guard, bounded correction engine, and deterministic manifest generator (ADK definition IMPLEMENTED in P-07.02; migration planner and manifest generators IMPLEMENTED in P-17) | Inward (depends on domain contracts) | Domain contracts | Direct external writes, UI | Provider-neutral |
...
| **Evidence Auditor** | `src/agents/evidence_auditor.py`, `src/audit/` | Independent semantic sufficiency review, neutral claim derivation, audit bundles, and deterministic fact reconciliation (ADK definition IMPLEMENTED in P-07.02; CCT-SEM-001 blind semantic audit boundary IMPLEMENTED in P-08.04; claim derivation, audit bundle builder, semantic auditor, and deterministic reconciler IMPLEMENTED in P-18) | Inward (depends on domain contracts, Evidence Record) | Domain contracts, Evidence Record (read) | Direct model fact rewrite, UI | Provider-neutral core with provider-specific model adapter |
| **Release Steward** | `src/agents/release_steward.py`, `integrations/github/github_adapter.py`, `src/release/` | Reversible handoff, safe PR drafting, 5-point idempotent writebacks, briefing generator, and isolated caller key boundaries (ADK definition IMPLEMENTED in P-07.02; bounded GitHub adapter, briefing generator, and receipt manager IMPLEMENTED in P-19). Consumes judge format from `docs/JUDGING_MAP.md` | Inward (depends on domain contracts, Change Passport) | Domain contracts, Change Passport, repository adapter port | Direct GitHub API calls bypassing adapter | Provider-neutral core with provider-specific adapter |
| **PubSub Timeline** | `src/evidence/pubsub_timeline.py` | Chronological execution and causal ordering (CCT-FLIGHT-001 IMPLEMENTED in P-09.05, P-20.01) | Inward (depends on domain contracts) | Domain contracts, event port/adapter | Direct PubSub SDK in core | Provider-neutral core + PubSub transport adapter |
...
| **Approval Compression** | `src/gate/compression.py`, `src/auth/approval_compression.py` | Autonomous vs escalation boundaries and compressed human decision packets (UIPATH-AUTH-001 IMPLEMENTED in P-14.02) | Inward (depends on domain contracts) | Domain contracts | Direct model SDK, UI | Provider-neutral |
| **ShadowLab Auth & Runner** | `src/shadowlab/runner.py`, `src/shadowlab/scenarios.py` | Isolated containerized simulated twin execution, fault recovery, and rollback verification (CCT-PREFLIGHT-001 IMPLEMENTED in P-13) | Inward (depends on domain contracts) | Domain contracts | Direct external execution, UI | Provider-neutral |
| **Bounded Gemini Model Client** | `src/core/gemini_client.py` | Canonical single bounded Gemini model client (IMPLEMENTED in P-08.01/P-08.05). Enforces exact model (`gemini-3.6-flash`), pinned API version (`v1beta1`), Vertex AI provider, explicit timeouts, bounded retries (wrapper-only authority, max 3; SDK retry disabled), token caps, immutable 4-category safety policy, non-secret telemetry with safe correlation IDs, rate-provenance cost estimation, project/demo budget evaluation, and canonical metrics artifacts. Calls Policy Guardian pre-SDK scan as non-bypassable privacy gate | Outward (depends on domain contracts, Policy Guardian, google-genai) | Domain contracts, Policy Guardian, google-genai SDK | Raw SDK client exposure to callers, caller-controlled model override, silent provider/model fallback | Provider-specific (Vertex AI / google-genai) |
| **Gemini Structured Output** | `src/core/gemini_structured_output.py` | Zero trust deserialization and contract validation (ZK-VALID-001 IMPLEMENTED in P-08.02). Enforces Policy Guardian allowlists during prompt construction | Inward (depends on domain contracts, Policy Guardian) | Domain contracts, Policy Guardian | Fixture data, UI | Provider-neutral (validates output regardless of model) |
...
| ADK agent skeleton implementation | P-07 | `DONE` (P-07.01–P-07.05) |
| Gemini integration and structured reasoning boundary | P-08 | `DONE` (P-08.01–P-08.05) |
...
| Firestore saga persistence data model | YES — designed in P-10.01 (`docs/P-10.01_FIRESTORE_DATA_MODEL.md`); repository implementation deferred to P-10.02 (`PENDING`) |
