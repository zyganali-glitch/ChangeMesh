...
| **Change Orchestrator** | `src/agents/change_orchestrator.py` | ADK intake skeleton implemented (P-07.01); definition metadata implemented (P-07.02); routing, saga coordination, recovery, multi-agent orchestration PLANNED | Inward (depends on domain contracts) | Domain contracts | Direct Firestore/PubSub client calls bypassing ports | Provider-specific (ADK) |
