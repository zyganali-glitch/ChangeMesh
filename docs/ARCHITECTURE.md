...
| **Change Orchestrator** | `src/agents/change_orchestrator.py` | ADK intake skeleton implemented (P-07.01); definition metadata implemented (P-07.02); deterministic routing/delegation implemented (P-07.03 via `src/agents/router.py`); saga coordination, recovery, fallback/concurrency PLANNED | Inward (depends on domain contracts) | Domain contracts, Deterministic Router | Direct Firestore/PubSub client calls bypassing ports | Provider-specific (ADK) |
