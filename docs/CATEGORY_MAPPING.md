...
| "Agent Identity (For zero-trust access control)" | Implemented via `Change Passport` capability tokens; no agent receives unrestricted credentials, relying on bounded identity. | `src/capability/passport.ts` (planned) |
| "Agent Gateway (for unified routing and policy enforcement)" | `Change Orchestrator` acts as the routing gateway, delegating to `Policy Guardian` for deterministic policy checks before execution. | `src/orchestration/router.ts` and `src/policy/` |
