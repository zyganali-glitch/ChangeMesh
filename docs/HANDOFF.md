...
- Complete P-09: `uv run python -m pytest tests/test_p09_01_topology.py tests/test_p09_02_pubsub_adapters.py tests/test_p09_03_retry_dead_letter.py tests/test_p09_04_local_event_bus.py tests/test_p09_05_pubsub_timeline.py -q` -> 76 passed.
- Canonical unit: `uv run python scripts/cmd.py unit` -> 1106 passed, 1 warning.
- Full suite: `uv run python -m pytest tests/` -> 1106 passed, 1 warning, 3 errors; **FAIL — known historical baseline GCP fixture debt** (`project` fixture in `tests/test_gcp_access.py`).
