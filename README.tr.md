...
- **Çalışma zamanı ürün ve ajan filosu geliştirmesi:** P-07 fazı tamamlandı (`DONE`). P-08 Gemini sınırı tamamlandı (`DONE`); P-08.01 sınırlı istemci, P-08.02 yapılandırılmış çıktı, P-08.03 deterministik girdi gizliliği/minimizasyonu, P-08.04 kör semantik gerçek ayrımı ve P-08.05 latency/token/retry/maliyet ölçümü tamamlandı. Bulut dağıtımı, kalıcılık ve P-12 Ajan Kayıt Defteri / Yetenek Pasaportu çalışma zamanı `PLANNED` durumundadır.
...
4. **Birim testleri çalıştırın:**
   ...
   ```
   *(P-05 kontratları, P-06 kontrolleri, P-07 ajan filosu/koordinasyonu ve P-08.01–P-08.05 Gemini sınır testleri dahil 1030 testi 0 çıkış koduyla çalıştırır; bir ADK kullanımdan kaldırma uyarısı kaydedilir).*
...
|---|---|---|---|
| `uv run python scripts/cmd.py unit` | Birim testleri çalıştır | Yerel deterministik test çalıştırma | `PASS` (1030 geçti, 1 uyarı) |
