...
> [!IMPORTANT]
> This is a **component dependency architecture**. Domain schemas and machine conventions are implemented and frozen in P-05. Implementation stack and dependency freeze are complete in P-06. The Change Orchestrator ADK skeleton and specialized agent fleet definitions (P-07), bounded Gemini structured output & privacy (P-08), Pub/Sub event backbone & causal DAG timeline (P-09), Firestore & in-memory saga state persistence (P-10), Memory Trust Layer (P-11), Capability Passport & Agent Registry (P-12), ShadowLab Change Rehearsal Twin (P-13), Reversibility Gate & Approval Compression (P-14), Impact Scout (P-15), Policy Guardian (P-16), Migration Engineer (P-17), Evidence Auditor (P-18), and Release Steward (P-19) are `IMPLEMENTED` (`DONE`). Phase P-20 (Orchestrator Saga, Recovery, and Long-Running Behavior) is `IN_PROGRESS` with P-20.00 (Long-running orchestration donor preflight) `DONE` and P-20.01 (End-to-End ChangeLifecycle Saga Orchestrator) `IMPLEMENTED` and `VERIFIED`. P-20.02 (Pause/Resume/Cancel/Compensation/Dead-Letter) remains `PENDING`. Live Google Cloud infrastructure deployments and runtime proof remain `NOT_RUN`, and all subsequent runtime and UI components remain `PLANNED` for their respective phases.
...
4. **Run unit tests:**
   ...
   ```
    *(Executes 1359 unit/contract tests across all implemented domain, agent, event, memory, capability, shadowlab, gate, policy, migration, audit, release, and saga modules with exit code 0; one ADK deprecation warning is recorded.)*
...
|---|---|---|---|
| `uv run python scripts/cmd.py unit` | Run unit tests | Local deterministic test execution | `PASS` (1359 passed, 1 warning) |
