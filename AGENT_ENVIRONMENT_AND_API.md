...
- **Retry Authority:** Exactly ONE retry authority owned by the ChangeMesh wrapper (`MAX_ATTEMPTS = 3`, exponential backoff with initial delay 0.5s, multiplier 2.0, max delay 2.0s). Retryable status codes restricted to `{429, 502, 503, 504}` and transient network errors. Non-retryable errors (400, 401, 403, 404, malformed input) fail immediately. Exhaustion raises `ModelRetryExhaustedError`.
...
- **Safety Settings:** Immutable enterprise safety settings (`CANONICAL_SAFETY_SETTINGS`) across 5 harm categories (`HARM_CATEGORY_HARASSMENT`, `HARM_CATEGORY_HATE_SPEECH`, `HARM_CATEGORY_SEXUALLY_EXPLICIT`, `HARM_CATEGORY_DANGEROUS_CONTENT`, `HARM_CATEGORY_CIVIC_INTEGRITY`) with threshold `HarmBlockThreshold.BLOCK_LOW_AND_ABOVE`. Blocked responses raise `ModelSafetyBlockedError` and fail closed.
- **Telemetry Boundary:** Typed `ModelCallTelemetry` capturing operational metadata (`call_id`, `model_id`, `provider`, `project`, `location`, timestamps, duration, attempts, outcome, status code, token counts, finish reason). STRICTLY ZERO credentials, ZERO API keys, ZERO prompt contents, ZERO response text.
...
| Unit tests (P-08.01 Gemini Client) | `python -m pytest tests/test_p08_01_gemini_client.py -v --tb=short` | `VERIFIED` | `PASS` (32 passed) | Local non-mutating test | 2026-08-16 |
| Unit tests (Full Suite) | `python -m pytest tests/` | `CLEAN_CHECKOUT_VERIFIED` | `FAIL` (942 passed, 3 errors: missing `project` fixture in `test_gcp_access.py`) | Local test suite execution | 2026-08-16 |
...
| Unit | `uv run python scripts/cmd.py unit` | `VERIFIED` | `PASS` (942 passed) | Non-mutating (`--ignore=tests/test_gcp_access.py`) | 2026-08-16 |
