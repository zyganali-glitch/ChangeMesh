...
| Unit tests (Full Suite) | `python -m pytest tests/` | `CLEAN_CHECKOUT_VERIFIED` | `FAIL` (759 passed, 3 errors: missing `project` fixture in `test_gcp_access.py`) | Local test suite execution | 2026-08-15 |
...
| Type-check | `uv run python scripts/cmd.py type-check` | `CLEAN_CHECKOUT_VERIFIED` | `FAIL` (Reports 2 errors in `test_gcp_access.py`) | Non-mutating (`mypy domain tests`) | 2026-08-15 |
| Unit | `uv run python scripts/cmd.py unit` | `CLEAN_CHECKOUT_VERIFIED` | `PASS` (759 passed) | Non-mutating (`--ignore=tests/test_gcp_access.py`) | 2026-08-15 |
